palette = ['#000000', '#8c4f4f', '#2f7a92', '#bacc94', '#9768b6'];
...
class Line {
	...
	draw() {
		...
		ctx.translate(-canvasDiagonal * 2 - 1, -canvasDiagonal);
		ctx.beginPath();
		ctx.rect(0, 0, canvasDiagonal * 2, canvasDiagonal * 2);
		ctx.restore();
...
function drawCanvas() {
	for (let i = 0; i < 3; i++) {
		let line = new Line(
			Math.random() * canvasSize, Math.random() * canvasSize, Math.random() * canvasSize, Math.random() * canvasSize, palette[0], paletteRandom(palette[0]));
		line.draw();
	}
}
drawCanvas();
